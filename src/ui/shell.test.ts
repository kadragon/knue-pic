...
describe('renderShell', () => {
  ...
  it('shows the update date only when the dataset provides one', () => {
    ...
    expect(withDate.textContent).toContain('최근 데이터 업데이트: 2026. 8. 1.');
  ...
  it('rewrites the update date in place instead of adding a second line', () => {
    ...
    expect(root.textContent).toContain('최근 데이터 업데이트: 2026. 8. 1.');
    expect(root.textContent).not.toContain('2026. 7. 1.');
