 ...
 * written here implicitly; and `local/no-monthkey-forgery` in `eslint.config.js` confines minting
 * one — by cast, by ambient declaration, or by assigning an untyped value to the annotation — to
 * `src/data/iso-date.ts`. That makes an accidental `MonthKey` here unlikely, not impossible: the
 * rule is a floor, and an unvalidated parse is still what `parseDataset` exists to stop. See that
 * file's `MonthKey` comment for what the rule does and does not reach.
 */
