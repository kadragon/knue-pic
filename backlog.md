...
- [ ] [debt] The `no-restricted-syntax` rules PR #32 added match the identifier *text*, so they
  ...
  `type Box<T = MonthKey> = T`, and `interface Mint { mint(x: string): MonthKey }` consumed through
  a `declare const`. Each was verified to reach `monthLabel` with `'1e3-08'`, which renders
  `1e3년 8월`. Closing them by selector is an arms race the last one wins outright — no text
  selector can see a type that is never named — so the real options are a typed lint rule
  (`@typescript-eslint` with type information) or accepting the floor. The limits are documented on
  `MonthKey` in `src/data/iso-date.ts`; this item is the decision, not a bug (source: PR #32
  contract QA, deliberately declined there) — `eslint.config.js`, `src/data/iso-date.ts`
...
- [ ] [docs] `src/stats/period.ts:23` says the half-open bound "keeps a `1m` window at 31 days
  ...
  (2026-03-31 → start 2026-02-28 → 31). `docs/architecture.md` was corrected in PR #32; this
  comment is where the same wrong number will be read from next (source: PR #32 contract QA,
  out of scope there) — `src/stats/period.ts`
